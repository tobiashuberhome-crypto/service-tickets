# Datei: resources\views\customer-portal-geiser\tickets\show.blade.php

> **Kommentar:** Automatischer Export des finalen Dateistands fuer Dokumentationszwecke.

- **Quelle:** `resources\views\customer-portal-geiser\tickets\show.blade.php`
- **Stand:** 2026-06-30 10:31:43
- **Typ:** blade
...
@section('content')
    <div class="page-header">
        ...
        <a class="btn secondary" href="{{ route('geiser-portal.dashboard') }}">Zurueck zur Ãœbersicht</a>
    </div>
        ...
        <div class="panel panel-body stack">
            ...
            @if ($isEditable)
                ...
                <div class="section">
                    ...
                    <div id="photo-preview-wrap" style="display:none; margin-top:12px;">
                        ...
                        <br>
                        <button type="button" id="btn-remove-photo" class="btn secondary" style="margin-top:8px; font-size:12px;">âœ• Neue Fotoauswahl entfernen</button>
                    </div>
            ...
            @else
                ...
                <div>
                    ...
                    <p>{{ $ticket->customerMachineProfile?->accessoriesSummary() ?: 'Keine EintrÃ¤ge' }}</p>
                </div>

                @if ($ticket->customerMachineProfile?->warranty_claimed)
                    <div class="alert alert-info">
                        âœ“ Garantie wird beansprucht
                    </div>
                @endif
            @endif
        </div>
    ...
    <div class="panel panel-body stack">
        ...
        @if ($isEditable)
            <form id="ticket-edit-form" method="post" action="{{ route('geiser-portal.tickets.update', $ticket) }}" class="stack" enctype="multipart/form-data">
                ...
                <button class="btn" type="submit" form="ticket-edit-form">Ã„nderungen speichern</button>
            </form>
        @else
            <div>
                            ...
                            <tbody>
                                @foreach ($ticket->serviceLines as $line)
                                    <tr>
                                        ...
                                        <td style="text-align: right;">{{ number_format($line->sales_price_snapshot, 2, ',', '.') }} â‚¬</td>
                                        <td style="text-align: right;"><strong>{{ number_format((float)$line->quantity * (float)$line->sales_price_snapshot, 2, ',', '.') }} â‚¬</strong></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                            ...
                            <tbody>
                                @foreach ($ticket->parts as $part)
                                    <tr>
                                        ...
                                        <td style="text-align: right;">{{ number_format($part->sales_price_snapshot, 2, ',', '.') }} â‚¬</td>
                                        <td style="text-align: right;"><strong>{{ number_format($part->totalNet(), 2, ',', '.') }} â‚¬</strong></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                        ...
                        <div class="grid" style="grid-template-columns: 1fr 2fr auto auto;">
                            ...
                            <div style="display: flex; gap: 0.5rem; align-items: flex-end;">
                                ...
                                <span style="white-space: nowrap; color: #666; font-size: 0.9em;">
                                    Ã— {{ number_format($line['unit_price'], 2, ',', '.') }} â‚¬
                                </span>
                            </div>
                            <div style="text-align: right; white-space: nowrap;">
                                <strong class="line-total">{{ number_format($line['line_total'], 2, ',', '.') }} â‚¬</strong>
                            </div>
                        </div>
            ...
            <div style="border-top: 1px solid #ddd; padding-top: 1rem; margin-top: 1rem;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <strong class="total-estimate" style="font-size: 1.2em;">
                        {{ number_format($estimateTotal ?? 0, 2, ',', '.') }} â‚¬
                    </strong>
                </div>
            </div>
        @else
                <div>
                    ...
                    <input value="{{ $ticket->customerMachineProfile?->repair_approval_limit ? number_format($ticket->customerMachineProfile->repair_approval_limit, 2, ',', '.') . ' â‚¬' : '-' }}" readonly>
                </div>
                        ...
                        <tbody>
                            @foreach ($estimateLines as $line)
                                <tr>
                                    ...
                                    <td style="text-align: right;">{{ number_format($line['quantity'], 2, ',', '.') }}</td>
                                    <td style="text-align: right;">{{ number_format($line['unit_price'], 2, ',', '.') }} â‚¬</td>
                                    <td style="text-align: right;"><strong>{{ number_format($line['line_total'], 2, ',', '.') }} â‚¬</strong></td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot>
                            <tr style="border-top: 2px solid #ddd;">
                                ...
                                <td style="text-align: right;"><strong>{{ number_format($estimateTotal ?? 0, 2, ',', '.') }} â‚¬</strong></td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
    ...
    @if (!$isEditable)
        <div class="alert alert-info">
            <strong>â„¹ Hinweis:</strong> Dieses Ticket ist nicht mehr offen. Aenderungen sind nicht moeglich. Sollten Sie Aenderungen benoetigen, kontaktieren Sie bitte den Support.
        </div>
    @endif
    ...
    <script>
        ...
        function updateEstimate() {
            ...
            Object.entries(lines).forEach(([fieldName, unitPrice]) => {
                ...
                if (lineTotalSpan) {
                    lineTotalSpan.textContent = formattedTotal + ' â‚¬';
                }
            });

            const totalSpan = document.querySelector('.total-estimate');
            if (totalSpan) {
                totalSpan.textContent = total.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' â‚¬';
            }
        }
        @endif
    </script>
