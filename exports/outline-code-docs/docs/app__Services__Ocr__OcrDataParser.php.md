# Datei: app\Services\Ocr\OcrDataParser.php

> **Kommentar:** Automatischer Export des finalen Dateistands fuer Dokumentationszwecke.

- **Quelle:** `app\Services\Ocr\OcrDataParser.php`
- **Stand:** 2026-06-27 20:11:13
- **Typ:** php
...
class OcrDataParser
{
     ...
     * Lieferzeit, Garantie, ZubehÃ¶r, handwritten description, and
     * "Bis zu einem Rechnungsbetrag in HÃ¶he von X EUR"
     */
     ...
     * e.g. "Record 730" â†’ machine_type = "Record 730"
    ...
    {
        // Same source as manufacturer on this form â€“ but map separately if needed
        return $this->findAfterLabel($text, [
            '/modell\s*[:\-]?\s*([^\n\r]{2,40})/i',
            '/maschinentyp\s*[:\-]?\s*([^\n\r]{2,40})/i',
            '/typ\s*[:\-]?\s*([a-z0-9][^\n\r]{1,30})/i',
     ...
     * e.g. "Bis zu einem Rechnungsbetrag in HÃ¶he von 250 EUR darf die Maschine..."
    ...
    protected function extractMaxCosts(string $text, array $lines): ?float
    {
        $patterns = [
            // Exact form sentence
            '/rechnungsbetrag[^0-9]{0,50}([0-9]+(?:[.,][0-9]{1,2})?)\s*(?:eur|â‚¬)/i',
            '/ohne\s+r.{0,3}cksprache[^0-9]{0,50}([0-9]+(?:[.,][0-9]{1,2})?)/i',
            '/h.{0,3}he\s+von\s+([0-9]+(?:[.,][0-9]{1,2})?)\s*(?:eur|â‚¬)/i',
            // Generic fallback
            '/max\.?\s*(?:kosten)?[:\s]*[â‚¬]?\s*([0-9]+(?:[.,][0-9]{1,2})?)/i',
            '/genehmigungslimit[:\s]*[â‚¬]?\s*([0-9]+(?:[.,][0-9]{1,2})?)/i',
        ];
    ...
    protected function extractDescription(string $text, array $lines): ?string
    {
        ...
        $skipMarkers = [
            'arbeitseinheit', 'servicegebÃ¼hr', 'servicegebuehr',
    ...
    {
        // Form has "â–¡ ja  X nein" or "X ja  â–¡ nein"
    ...
    {
        $items = [
            'presser_foot'   => ['nÃ¤hfuÃŸ', 'nahfus', 'nahfuss', 'naehfuss'],
            'bobbin_case'    => ['spulenkapsel'],
            'bobbin'         => ['unterfadenspule', 'unterfaden'],
            'power_cable'    => ['kabel'],
            'foot_pedal'     => ['fuÃŸanlasser', 'fussanlasser', 'anlasser'],
            'case'           => ['koffer'],
        ];
        ...
        foreach ($items as $key => $keywords) {
            foreach ($keywords as $kw) {
                ...
                if (preg_match('/[xXâœ“]\s*' . preg_quote($kw, '/') . '|' . preg_quote($kw, '/') . '\s*[xXâœ“]/i', $lower)) {
                    $found[] = $key;
                    break;
                }
            }
