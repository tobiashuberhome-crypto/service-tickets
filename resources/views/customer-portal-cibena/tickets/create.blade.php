...
@section('content')
    ...
    <form method="post" action="{{ route('cibena-portal.tickets.store') }}" class="panel panel-body stack" style="max-width: 980px" enctype="multipart/form-data" id="ticket-form">
        @csrf
        ...
        <div class="button-row">
            ...
            <a class="btn secondary" href="{{ route('cibena-portal.dashboard') }}">Abbrechen</a>
        </div>
    </form>
...
(() => {
    ...
    async function lookupSerial(force = false) {
        ...
        try {
            const response = await fetch(`{{ route('cibena-portal.machine-profiles.lookup') }}?serial_number=${encodeURIComponent(serial)}`, {
